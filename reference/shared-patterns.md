...
> **⚠️ 关键检查点**：若需要计划上下文，请通过 `plan-path=<path>` 显式传递计划文件路径。
...
若需要计划上下文，请通过 `plan-path=<path>` 显式传递：
- 使用 Read 验证文件存在并判断与当前任务的相关性
- 若相关，在 PROMPT 中包含计划路径
- 若未指定 `plan-path`，则不传递计划上下文
...
在执行 Codex 协作命令时，可通过 `plan-path=<path>` 显式指定计划文档，为 Codex 提供更丰富的上下文。

### 计划文件处理

若 `$ARGUMENTS` 包含 `plan-path=<path>`：
1. 使用 Read 工具验证文件存在
2. 判断计划与当前任务的相关性
3. 若相关，记录绝对路径用于传递给 Codex

若未指定 `plan-path`，则不传递计划上下文，正常执行工作流。
...
- 文件路径、代码范围或 git diff
- 需求目标或问题症状
- 可选参数（如 `focus`、`session`、`plan-path`）

**计划文件处理**：
若 `plan-path=<path>`：验证并使用指定的计划文件
