...
**场景**：已有实现计划 `.claude/plans/auth-feature.md`，开发完成后进行代码审查

**计划文件内容**（`.claude/plans/auth-feature.md`）：
```markdown
# 用户认证功能计划
...
```
## 结论概述
发现 1 个 High 问题（账户锁定未实现），2 个验收标准未满足。

## 问题清单

### High: 账户锁定逻辑缺失
**位置**：src/auth/login.py:45
**问题**：登录失败后未记录失败次数，缺少账户锁定逻辑
**关联验收标准**：❌ 登录失败 5 次后锁定账户 15 分钟

## 计划关联

### 验收标准检查
- ✅ 支持邮箱+密码登录 - login() 函数已实现
- ✅ 密码使用 bcrypt 加密存储 - hash_password() 使用 bcrypt
- ❌ 登录失败 5 次后锁定账户 15 分钟 - 未实现
- ✅ JWT token 有效期 24 小时 - create_token() 设置 exp=24h

Codex session: 019ba7a9-8b52-7723-971c-90454123c720
```
...
**用户输入**：
```
/codex-plan 帮我完善这个缓存系统的计划
```

**计划文件内容**（`.claude/plans/cache-system.md`）：
```markdown
# 缓存系统计划
...
Codex session: 019ba7a9-8b52-7723-971c-90454123c721
```

---

### 示例 5：Debug 时参考已有排查进展

**场景**：已进行过初步排查，有排查记录文件

**计划文件内容**（`.claude/plans/debug-memory-leak.md`）：
```markdown
# 内存泄漏排查进展

## 已验证假设
- ❌ 事件监听器未移除 - 已检查，监听器正确移除
- ❌ 闭包引用 - 已检查，无循环引用
...
### 聚焦验证
基于排查记录，优先验证：
1. 数据库连接池泄漏 - 检查 pool.release() 调用
2. 大对象未释放 - 检查 Buffer/Stream 处理

## 根因假设
最可能的原因：数据库连接池配置的 max 过大，且未正确释放连接。

Codex session: 019ba7a9-8b52-7723-971c-90454123c722
```
